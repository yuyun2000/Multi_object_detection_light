...
# premodel = tf.keras.models.load_model("./mbnet_epoch_149.h5")
# （3）主干网络
...
def mobilenet_v1( input_shape, alpha, depth_multiplier, dropout_rate):
    ...
    x = conv_block(x, 64, alpha)
    x = conv_block(x, 128, alpha)

    x = depthwise_conv_block(x, 224, alpha, depth_multiplier)
    x = depthwise_conv_block(x, 224, alpha, depth_multiplier)
    x = depthwise_conv_block(x, 128, alpha, depth_multiplier)
    x = depthwise_conv_block(x, 64, alpha, depth_multiplier)
    x = depthwise_conv_block(x, 64, alpha, depth_multiplier)
    x = depthwise_conv_block(x, 32, alpha, depth_multiplier)
    x = depthwise_conv_block(x, 64, alpha, depth_multiplier)
    ...
    x = layers.Conv2D(64, (1, 1), 1, activation='relu', name='head3')(x)
    x = layers.Conv2D(16, (1, 1), 1, activation='relu', name='head4')(x)
    x = layers.Conv2D(2, (1, 1), 1, activation='relu', name='logits')(x)
